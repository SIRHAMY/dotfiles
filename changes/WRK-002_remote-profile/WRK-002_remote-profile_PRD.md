...
- **Per-machine `conf.d/local/` slot for remote-only secrets, identity, or proxy config.** Includes shell secrets, work credentials, proxy config, **and per-env git identity (work vs. personal `user.email`)**. Deferred unless a concrete env requires it. Today the `git` stow package's identity is shared across all envs; GitHub attributes commits to whichever account has the email registered, so personal-on-work usually works. If a work GitHub org's SSO/compliance ever rejects a commit, cut a follow-up change.
...
15. **Per-env git identity (work email vs. personal email) is deferred.** GitHub attributes commits to whichever account has the commit email registered, so personal emails on work boxes still attribute correctly in most cases. If a work GitHub org's SSO/compliance check ever rejects a commit, cut a follow-up change for a `conf.d/local/`-style include slot. Not v1.
